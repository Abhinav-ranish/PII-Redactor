...
export const DEFAULT_ENTITIES: EntityType[] = [
  'PERSON','EMAIL_ADDRESS','PHONE_NUMBER','US_SSN','CREDIT_CARD','DATE_TIME','IP_ADDRESS','URL','ADDRESS','POSTAL_CODE','LOCATION','MEDICAL_RECORD_NUMBER'
];

export const DEFAULT_MASK: Partial<Record<EntityType,string>> = {
  PERSON:'[NAME]', EMAIL_ADDRESS:'[EMAIL]', PHONE_NUMBER:'[PHONE]', US_SSN:'[SSN]', CREDIT_CARD:'[CARD]', DATE_TIME:'[DATE]',
  IP_ADDRESS:'[IP]', URL:'[URL]', ADDRESS:'[ADDRESS]', POSTAL_CODE:'[ZIP]', LOCATION:'[LOCATION]', MEDICAL_RECORD_NUMBER:'[MRN]'
};

export type RedactResult = { redacted_text: string; entities: EntityMatch[]; policy: Policy };

export function redactText(text: string, policy?: Policy): RedactResult {
  const pol: Policy = {
    mode: policy?.mode ?? 'delete',
  ...
  if (pol.mode === 'delete') {
    ...
    for (const e of sorted) {
      ...
      redacted_text = redacted_text.slice(0, e.start) + token + redacted_text.slice(e.end);
    }
  }
